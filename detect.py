...
@torch.no_grad()
def run(weights=ROOT / 'weights/yolov5n.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # per image
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if names[c]:
                            if names[c] == 'Alert':
                                ...
                                s_img = cv2.imread("icons/warning.jpg")
            ...
            if view_img:
                display_h, display_w = im0.shape[:2]
                target_w = 800
                scale_factor = target_w / display_w
                new_h = int(display_h * scale_factor)
                im0_resized = cv2.resize(im0, (target_w, new_h))
                
                cv2.imshow(str(p), im0_resized) # Show the resized image
                k = cv2.waitKey(1) & 0xFF # Make sure this is 1, NOT 0
